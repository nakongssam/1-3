...
exam_date = st.sidebar.date_input(
    "시험 시작일", value=datetime.date.today() + datetime.timedelta(days=14)
)
...
if d_day > 0:
    ...
    progress_percent = (
        max(0, min(100, int((30 - d_day) / 30 * 100))) if d_day < 30 else 0
    )
    ...
    st.success(
        f"🎉 드디어 시험 당일입니다! {st.session_state.motto} 실력 발휘 제대로 하고 오세요!"
    )
    st.balloons()
else:
...
new_motto = st.text_input(
    "나에게 자극이 되는 한 마디를 적어보세요:", value=st.session_state.motto
)
...
with st.form(key="todo_form", clear_on_submit=True):
    ...
    submit_button = st.form_submit_button(label="계획 추가")
...
if st.session_state.tasks:
    ...
    st.write(
        f"📊 **현재 계획 달성률:** {achievement_rate}% ({completed_count}/{total_tasks})"
    )
    ...
    st.info(
        "등록된 공부 계획이 없습니다. 위의 양식에서 시험 계획을 추가해보세요!"
    )
